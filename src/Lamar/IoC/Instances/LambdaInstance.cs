...
{
    public LambdaInstance(Type serviceType, Func<IServiceProvider, object> factory, ServiceLifetime lifetime) : base(
        serviceType, factory, lifetime)
    {
    }
...
public class LambdaInstance<TContainer, TReturn> : Instance
{
    ...
    public override string WhyRequireServiceProvider(IMethodVariables method)
    {
        return $"The scoping is {Lifetime}, so a Lambda registration requires" + Environment.NewLine +
               "the usage of a nested container for resolution for correct scoping." + Environment.NewLine +
